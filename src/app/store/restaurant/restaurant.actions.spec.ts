...
describe('Search Actions', () => {
  ...
  it('should create search action with query', () => {
    const query = 'Pho 10';
    const action = findBySearch({ query });

    expect(action.type).toBe('[Search] Find By Search');
    expect(action.query).toBe(query);
  });

  it('should create searchSuccess action with results mapped from mockSearchResponse', () => {
    const results: RestaurantModel[] = mockApiResponse.features.map(f => RestaurantModel.fromFeature(f));

    const action = findBySearchSuccess({ resultsRestaurant:results });

    expect(action.type).toBe('[Search] Find By Search Success');
    expect(action.resultsRestaurant.length).toBeGreaterThan(0);
    expect(action.resultsRestaurant[0].name).toBe('Pho 10');
    
  });

  it('should create searchFailure action with error', () => {
    const error = { message: 'Something went wrong' };
    const action = findBySearchFailure({ error:error.message });

    expect(action.type).toBe('[Search] Find By Search Failure');
